...
function getlAllBlogPostsDetails () {
   $conn = getDB();
   $duzenle=$_GET["deger"];
   $stmt = $conn->prepare("SELECT * FROM Blog WHERE idBlog = $duzenle");
   $stmt->execute();
   $result = $stmt->fetchAll();
   closeDB($conn);
   return $result;
...
# Add Blog:
function addNewBlog($title, $text, $category) {
    if (!isset($_SESSION)) {
        session_start();
    }
    ...
    $conn = getDB();
    $stmt = $conn->prepare("INSERT INTO Blog (title, text, category, date) VALUES (:title, :text, :category, :Member_idMember, :date");
